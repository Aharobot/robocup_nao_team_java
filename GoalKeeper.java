...
public class GoalKeeper extends Player {
  ...
  private Motion sideStepLeftMotion, sideStepRightMotion, forwards50Motion, backwardsMotion,shootingMotion,push;
  ...
  public GoalKeeper(int playerID, int teamID) {
    ...
    // push right left motion 
    /* shoulder roll and pitch yaw up, knee angle-> left 0**/  
  ...
  private void getUpIfNecessary() {
	 double goAcc[] = getAcc();
	 if (goAcc[2] < 5.0 && goAcc[0] < - 4.0)
         standUpFromFront();
     if (goAcc[2] < 5.0 && goAcc[0] >  4.0)
         standUpFromBack();
     if (goAcc[2] < 5.0 && goAcc[1] < - 4.0)
         returnFromSide();
     if (goAcc[2] < 5.0 && goAcc[1] > 4.0)
         returnFromSide();
         headScan();
