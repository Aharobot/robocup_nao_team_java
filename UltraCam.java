...
public class UltraCam extends Camera{
    ...
    public UltraCam(String name) {
        ...
        private int[] image2pixels(int image[]) {
 
            for (int i = 0; i < image.length; i++) {
                if((Camera.pixelGetRed(image[i])+Camera.pixelGetGreen(image[i])+Camera.pixelGetBlue(image[i]))/3 > 250){ 
                    image[i] = 0xffffff;
                }
                else 
                    image[i] = 0x000000;
                int r = (image[i] & 0xff0000) >> 16;
                int g = (image[i] & 0xff00) >> 8;
                int b = image[i] & 0xff;
                image[i] =  (int) (0.29799999999999999D * (double) r
                        + 0.58599999999999997D * (double) g
                        + 0.113D * (double) b);            
            }
            return image;
        }
